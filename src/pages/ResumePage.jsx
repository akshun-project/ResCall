...
const userId = user?.id;
    ...
    return lines.map((line, index) => {
      ...
      if (line.includes(":") && !line.startsWith("-")) {
        return (
          <h3
            key={index}
            className="text-lg font-semibold text-indigo-400 mt-5"
          >
            {line}
          </h3>
        );
 ...
 const analyzeResume = async () => {
  ...
  if (!userId) {
    return alert("User not loaded. Please wait...");
  }

  // 📄 Ensure resume exists
  if (!resumeText.trim()) {
    return alert("Upload your resume first.");
  }
  ...
  try {
    const prompt = `
You are an expert ATS and technical interviewer.
...
RULES:
- Return ONLY JSON
- Max 5 missing_keywords
- Max 5 missing_skills
- Max 6 questions
- Keep insight under 30 words
...
`;
    ...
    try {
      ...
      const fallbackData = {
        ats: {
          score: 60,
          missing_keywords: ["React", "APIs"],
          quick_fixes: ["Improve bullet points", "Add measurable results"]
        },
        skills: {
          level: "Intermediate",
          missing_skills: ["System Design", "Backend"]
        },
        questions: [
          { type: "technical", question: "Explain useEffect in React" },
          { type: "hr", question: "Tell me about yourself" }
        ],
        insight: "Good base. Improve backend depth and measurable impact."
      };

      saveData(`aiData_${userId}`, fallbackData);

      setAnalysis("Server busy ⚠️ Using fallback...");
    ...
    // 🔥 Fallback if API fails completely (503 etc.)
    const fallbackData = {
      ats: { score: 55, missing_keywords: [], quick_fixes: [] },
      skills: { level: "Intermediate", missing_skills: [] },
      questions: [],
      insight: "Temporary issue. Please try again."
    };

    saveData(`aiData_${userId}`, fallbackData);

    setAnalysis("Server error ⚠️ Showing fallback...");
  ...
  return (
    ...
    {loading ? (
  <p className="text-gray-400 animate-pulse">Analyzing resume…</p>
) : analysis ? (
  <p className="text-green-400 font-medium">
    Resume analyzed successfully ✅ Redirecting...
  </p>
) : (
  <p className="text-gray-400">Upload a resume to start.</p>
)}
        </motion.div>
      </div>
    </div>
